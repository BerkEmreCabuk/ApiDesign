...
namespace ApiDesign.Api.Controllers.V2
{
    /// <summary>
    /// Versiyon 1 ile aynı endpointleri içermektedir
    /// Örnek olması için eklenmiştir
    /// Major versiyonlarda ayrı Controller olması daha uygun olur
    /// </summary>
    [ApiVersion("2.0")]
    [ApiController]
    ...
    public class CategoriesController : ControllerBase
    {
        ...
        public IActionResult Get(long id)
        {
            try
            {
                var category = CommonSeedModel.CategoriesV2.FirstOrDefault(x => x.Id == id);
                if (category == null)
        ...
        public IActionResult GetList()
        {
            try
            {
                var response = new BaseResponseModel<List<CategoryResponseModel>>(CommonSeedModel.CategoriesV2);
                return Ok(this.CreateLinksForCategory(response));
            }
            catch (Exception)
            {
        ...
        [HttpGet(Name = nameof(GetList_Version2_1))]
        [ApiVersion("2.1")]
        [ProducesResponseType(typeof(BaseResponseModel<List<CategoryResponseModel>>), 200)]
        [ProducesResponseType(400)]
        public IActionResult GetList_Version2_1()
        {
            try
            {
                var response = new BaseResponseModel<List<CategoryResponseModel>>(CommonSeedModel.CategoriesV2);
        ...
        public IActionResult Create([FromBody] CategoryCreateRequestModel model)
        {
            try
            {
                var isExist = CommonSeedModel.CategoriesV2.Exists(x => x.Name == model.Name && x.Status == Status.ACTIVE);
                if (isExist)
                ...
                CommonSeedModel.CategoriesV2.Add(category);
                var response = new BaseResponseModel<CategoryResponseModel>(category);
        ...
        public IActionResult Update([FromBody] CategoryUpdateRequestModel model)
        {
            try
            {
                var category = CommonSeedModel.CategoriesV2.FirstOrDefault(x => x.Id == model.Id);
                if (category == null)
                    return NotFound(new BaseResponseModel("Kategori bulunamadı."));
        ...
        public IActionResult AllFollow()
        {
            try
            {
                var isExist = CommonSeedModel.CategoriesV2.Exists(x => x.Status == Status.ACTIVE);
                if (!isExist)
        ...
        public IActionResult Unfollow(long id)
        {
            try
            {
                var isExist = CommonSeedModel.CategoriesV2.Exists(x => x.Id == id && x.Status == Status.ACTIVE);
                if (!isExist)
                    return NotFound(new BaseResponseModel("Kategori bulunamadı."));
        ...
        public IActionResult Delete(long id)
        {
            try
            {
                ...
                CommonSeedModel.CategoriesV2.Remove(category);
